...
#***************************************************************
#	GFDMmix for CS - Skill Data Edit
...
begin
	...
	when 'get'				# 編集フォーム立ち上げ
		...
		s_edit = GfDmMix::SkillEditFormMaker.new(cgi, sp_access?(cgi), session)
		html << s_edit.to_html

	when 'post'				# 編集内容登録確認/実行
		s_reg = GfDmMix::SkillRegister.new(cgi, sp_access?(cgi), session)
		s_reg.execute
		html << s_reg.to_html
	end
