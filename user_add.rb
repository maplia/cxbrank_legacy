...
#***************************************************************
#	GFDMmix for CS - User Registration
...
begin
	cgi = CGI.new
	session = GfDmMix::UserAddSession.new(cgi)
	if (cgi.request_method.downcase == 'post') and (not session.alive?)
		print respond_to_http_request(cgi, GfDmMix::UserAddSessionExecutor.new(cgi, session))
		exit
	end
