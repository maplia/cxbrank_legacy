...
module GfDmMix
	class SkillDatabase < Database
		...
		def get(music, ignore_lock=false)
			...
			skill = make_skill_item_from_result_hash(@user, hash, ignore_lock)
		...
		def get_all_skill(ignore_lock=false)
			...
			result.each_hash(true) do |hash|
				mid = hash['skills.music_id'].to_i
				skills[mid] = make_skill_item_from_result_hash(@user, hash, ignore_lock)
			end

			musics.each do |music|
				unless skills.has_key?(music.mid)
					skills[music.mid] = SkillItem.make_empty_item(@user, music)
				end
			end

			return SkillItemSet.new(@user, skills.values, ignore_lock)
		end

		def get_all_user_skill(music)
			skills = Array.new

			query = ''
			query << 'select * from skills '
			query << 'inner join musics on skills.music_id = musics.id '
			query << "where musics.text_id = '#{music.text_id}';"
			result = @db.query(query)
			u_db = UserDatabase.new
			result.each_hash(true) do |hash|
				user = u_db.get(hash['skills.user_id'])
				skills << make_skill_item_from_result_hash(user, hash)
			end
			u_db.close
		...
		private
		def make_skill_item_from_result_hash(user, hash, ignore_lock=false)
			...
			return SkillItem.new(user, hash['skills.id'].to_i, music,
	...
	class SkillItem
		...
		def initialize(user, sid, music, bsc_skill, adv_skill, ext_skill, comment)
			@user = user
			@sid = sid
			@music = music
		...
		def self.make_empty_item(user, music)
			...
			return SkillItem.new(user, nil, music, bsc_skill, adv_skill, ext_skill, '')
		end
		...
		def to_html_for_rank(row, mobile)
			# ここでの出力結果は、スキル表の中の1行分のHTMLテキストになる
			if @point == 0.0
				# 最高ポイントが0であるものは表示対象としない
				return ''
			else
				return @max_skill.to_html_for_rank(@user, row, mobile, @comment)
			end
		end
	end
	...
	class SkillScoreItem
		...
		def to_html_for_rank(user, row, mobile, comment)
			if mobile
				template = 'template/mobile/skill_rank_list_item.html.erb'
			else
				template = 'template/skill_rank_list_item.html.erb'
			end
